...
// Variables de gestion du multijoueur
u32 mp_receivedPackets = 0;                // Compte des paquets reçus
bool mp_duplicateSendProtection = false;   // Protection contre les envois en double, potentiellement moins nécessaire avec TCP
static s8 netStage = 0;                    // Étape actuelle de la connexion réseau
bool mp_isSyncing = false;                 // Indique si la synchronisation est en cours
bool mp_foundSyncer = false;               // Indique si un synchroniseur a été trouvé
bool mp_completeSyncs[6];                  // Tableaux pour suivre les synchronisations complètes
bool mSaveContextInit = false;             // Indique si le contexte de sauvegarde a été initialisé
static u16 fullSyncerID = 0;               // ID du client pour la synchronisation complète

// Variables de réseau
u32* mBuffer;                             // Tampon pour la réception des données réseau
size_t mBufSize;                          // Taille du tampon
u8 data_channel = 1;                      // Canal de données, à ajuster si plusieurs canaux sont utilisés
udsBindContext bindctx;                   // Contexte de liaison, à ajuster pour TCP


static void Multiplayer_Sync_Init();
static void Multiplayer_Sync_SharedProgress();
static void Multiplayer_SendPacket(int socket_desc, u16 packageSize);
...
static u8 IsSendReceiveReady(void) {
    // Vérifie si la fonctionnalité multijoueur est activée
    if (gSettingsContext.mp_Enabled == OFF) {
        return 0;
    }

    // Vérifie si l'étape du réseau indique que la connexion est prête
    // Avec TCP, on suppose que netStage >= 3 signifie que la connexion est établie
    // et prête à envoyer/recevoir des données
    return netStage >= 3;
}
...
void Multiplayer_Run(void) {
    ...
    switch (netStage) {
        case 0:
            ...
            if (initTimer >= 30) {
                // Initialize TCP context if needed
                result = tcpInit(); // Hypothetical function to initialize TCP
                if (R_FAILED(result)) {
                    netStage = -1;
                    return;
                }
            ...
            break;
        case 1:
            // Attempt to connect to a server or host a server
            if (netScanChecks < (playingOnCitra ? 30 : 3)) {
                netScanChecks++;

                // Attempt to connect to a predefined server address
                result = tcpConnectToServer("127.0.0.1", 12345); // Hypothetical function and address
                ...
                // Set up a server if no connection was made
                result = tcpHostServer("127.0.0.1", 12345); // Hypothetical function and address
                if (R_FAILED(result)) {
                    netStage = -1;
                    return;
                }
        ...
        case 2:
            ...
            mBufSize = TCP_DATAFRAME_MAXSIZE; // Adjust size for TCP
            mBuffer  = SystemArena_Malloc(mBufSize);
...
typedef struct {
    u8 total_clients; // Nombre total de clients connectés
    // Ajoutez d'autres champs si nécessaire
} tcpConnectionStatus;


s8 Multiplayer_PlayerCount(void) {
    // Hypothetical function to get the number of connected clients in a TCP server
    // Replace `tcpGetConnectionStatus` with the actual function provided by your TCP library
    tcpConnectionStatus status;
    if (R_SUCCEEDED(tcpGetConnectionStatus(&status))) {
        return status.total_clients; // Adjust based on your TCP library's API
    }
    return -1;
}
...
void Multiplayer_Send_GhostData(void) {
    ...
    ghostData.childTunicColor.r = gSettingsContext.childTunicColor.r;
    ghostData.childTunicColor.g = gSettingsContext.childTunicColor.g;
    ghostData.childTunicColor.b = gSettingsContext.childTunicColor.b;
    ghostData.childTunicColor.a = gSettingsContext.childTunicColor.a;

    // memcpy(&ghostData.childTunicColor, &gSettingsContext.childTunicColor, sizeof(ghostData.childTunicColor));
    memcpy(&ghostData.kokiriTunicColor, &gSettingsContext.kokiriTunicColor, sizeof(ghostData.kokiriTunicColor));
    memcpy(&ghostData.goronTunicColor, &gSettingsContext.goronTunicColor, sizeof(ghostData.goronTunicColor));
    memcpy(&ghostData.zoraTunicColor, &gSettingsContext.zoraTunicColor, sizeof(ghostData.zoraTunicColor));
...
void Multiplayer_Send_FullSyncRequest(u8 neededPacketsMask) {
    ...
    // Clear the buffer and prepare it for the sync request
    memset(mBuffer, 0, mBufSize);
    u8 memSpacer = PrepareSharedProgressPacket(PACKET_FULLSYNCREQUEST);

    // Store the neededPacketsMask in the buffer
    mBuffer[memSpacer++] = neededPacketsMask;

    // Determine the target node ID for sending the packet
    struct sockaddr_in targetAddress;
    if (neededPacketsMask == 0) {
        // Broadcast to all nodes
        targetAddress.sin_family = AF_INET;
        targetAddress.sin_addr.s_addr = INADDR_BROADCAST; // Broadcast address
        targetAddress.sin_port = htons(PORT); // Port number to be replaced with actual port
    } else {
        // Send to a specific node
        targetAddress.sin_family = AF_INET;
        targetAddress.sin_addr.s_addr = inet_addr(fullSyncerIP); // Replace with actual IP
        targetAddress.sin_port = htons(PORT); // Port number to be replaced with actual port
    }

    // Send the packet
    sendto(socketFD, mBuffer, memSpacer, 0, (struct sockaddr*)&targetAddress, sizeof(targetAddress));
}
...
void Multiplayer_Send_RupeeChange(int socket_desc, s16 diff) {
    if (socket_desc < 0) {
        return;
    }
    ...
    ssize_t bytes_sent = send(socket_desc, mBuffer, memSpacer, 0);
    if (bytes_sent < 0) {
        printf("\x1b[3;0HFailed to send Rupee change data");
    }
}


void Multiplayer_Receive_RupeeChange(int socket_desc) {
    ...
    ssize_t bytes_received = recv(socket_desc, mBuffer, sizeof(mBuffer), 0);
    if (bytes_received <= 0) {
        printf("\x1b[3;0HFailed to receive Rupee change data or connection closed");
        return;
    }
...
void Multiplayer_Send_AmmoChange(int socket_desc, u8 slot, s8 diff) {
    if (!IsSendReceiveReady()) {
        return;
    }
    ...
    ssize_t bytes_sent = send(socket_desc, mBuffer, memSpacer, 0);
    if (bytes_sent < 0) {
        printf("\x1b[3;0HFailed to send ammo change data");
    } else if (bytes_sent != memSpacer) {
        printf("\x1b[3;0HPartial send of ammo change data");
    }
}
...
void Multiplayer_Receive_AmmoChange(u16 senderID) {
    ...
    if (gSettingsContext.mp_SharedProgress == ON) {
        if (IsInSameSyncGroup()) {
            ...
            // Optionally, handle max ammo here based on game logic
            // Example:
            static u8 maxAmmo[] = { 30, 20, 10 };  // Adjust based on slot types
            u8 maxAmmoLimit = maxAmmo[slot];
            if (mSaveContext.ammo[slot] > maxAmmoLimit) {
                mSaveContext.ammo[slot] = maxAmmoLimit;
            }
        }
        ...
        static u8 maxAmmo[] = { 30, 20, 10 };  // Adjust based on slot types
        u8 maxAmmoLimit = maxAmmo[slot];
        if (gSaveContext.ammo[slot] > maxAmmoLimit) {
            gSaveContext.ammo[slot] = maxAmmoLimit;
        }
        prevAmmo[slot] = gSaveContext.ammo[slot];
    }
}
...
//     printf("\x1b[3;0HConnected!");
//     while (aptMainLoop()) {
//         hidScanInput();
//         u32 kDown = hidKeysDown();
//         if (kDown & KEY_START) break;
//         Multiplayer_ReceivePackets(socket_desc);
//         gfxFlushBuffers();
//         gfxSwapBuffers();
//         gspWaitForVBlank();
//     }
...
static void Multiplayer_SendPacket(int socket_desc, u16 packageSize) {
    send(socket_desc, mBuffer, packageSize, 0);
}

void Multiplayer_ReceivePackets(socket_desc) {
    if (!IsSendReceiveReady()) {
        return;
    }
    ...
    ssize_t actual_size;
    while ((actual_size = recv(socket_desc, mBuffer, sizeof(mBuffer), 0)) > 0) {
        Multiplayer_UnpackPacket(socket_desc); // Vous pouvez adapter cette fonction pour recevoir des données spécifiques
...
static void Multiplayer_UnpackPacket(u16 senderID) {
    ...
    // Assurez-vous que ces fonctions correspondent à celles de l'enum PacketIdentifier
    ...
    // Vérifiez que l'identifiant du paquet est valide et qu'il est dans les limites
    if (identifier < sizeof(receive_funcs) / sizeof(receive_funcs[0])) {
        receive_funcs[identifier](senderID);
    } else {
        // Optionnel : Gérez les erreurs pour les identifiants de paquets inconnus
        printf("Erreur : Identifiant de paquet inconnu %u reçu de %u\n", identifier, senderID);
    }
}
