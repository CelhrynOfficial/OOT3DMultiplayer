...
void EnLinkPuppet_Init(EnLinkPuppet* this, GlobalContext* globalCtx) {
    ...
    if (gSettingsContext.customTunicColors == ON) {
        ...
        if (exObjectBankIdx >= 0) {
            cmabMan = ZAR_GetCMABByIndex(&rExtendedObjectCtx.status[exObjectBankIdx].zarInfo, (gSaveContext.linkAge == 0) ? TEXANIM_LINK_BODY_1 : TEXANIM_CHILD_LINK_BODY_1);
        }
...
void EnLinkPuppet_UpdateTunicColor(EnLinkPuppet* this, GlobalContext* globalCtx) {
    s16 exObjectBankIdx = Object_GetIndex(&rExtendedObjectCtx, OBJECT_CUSTOM_GENERAL_ASSETS);
    ...
    if (this->ghostPtr->ghostData.age == 1) {
        cmabManager = ZAR_GetCMABByIndex(&rExtendedObjectCtx.status[exObjectBankIdx].zarInfo, TEXANIM_CHILD_LINK_BODY_1);
        redOffset   = 0x70;
        greenOffset = 0x88;
        blueOffset  = 0xA0;
    ...
    if (this->ghostPtr->ghostData.age == 1) {
        ...
        Color_RGBAf color = this->ghostPtr->ghostData.childTunicColor;

        *(f32*)(cmabChunk + redOffset)   = color.r;
        *(f32*)(cmabChunk + greenOffset) = color.g;
        *(f32*)(cmabChunk + blueOffset)  = color.b;
        *(f32*)(cmabChunk + blueOffset)  = color.b;
    } else {
        // handle when adult
    }
